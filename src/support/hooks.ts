...
// Cucumber-style hooks (registered by requiring this file via `cucumberOpts.require`)
import { After } from '@cucumber/cucumber';

After(async function (scenario) {
    try {
        // example action: take a screenshot after each scenario (useful for debugging)
        await (browser as WebdriverIO.Browser).saveScreenshot(`./allure-results/screenshot-${Date.now()}.png`);
    } catch (err) {
        // swallow errors to not interrupt test shutdown
